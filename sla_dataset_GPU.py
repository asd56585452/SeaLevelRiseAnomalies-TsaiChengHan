...
class SLADataset(Dataset):
    ...
    def _preload(self):
        """遍历所有索引，调用 __getitem__，将数据预加载到内存或 GPU"""
        for idx in range(len(self.all_dates)):
            data = self._load_single(idx)
    ...
    def _load_single(self, idx):
        ...
        sla = torch.tensor(np.nan_to_num(sla, nan=self.fill_value), dtype=torch.float32, device="cuda")

        # 构建标签数据
        labels = torch.zeros(len(self.locations), dtype=torch.float32, device="cuda")  # 默认所有站点为 0（非异常）
